...
define('utils/aopproxy', ['utils', 'utils/executionevent'], function (utils, execEvent) {
        ...
        /**
         * Creates a proxy object from the instance of a passed Object. This proxy Object
         * duplicates the methods of the passed Object instance, omitting any functions reserved
         * for the proxy Object and any functions which are from the Object prototype.
         *
         * Once the duplication is completed, the new proxy object is returned with functions
         * available to fire off callbacks (labeled aspects) during the execution of any of the
         * main functions for the object.
         *
         * @class AOPProxy
         * @param object {Object} the object instance to proxy.
         * @constructor
         */
        function AOPProxy(object) {
                ...
                if (
                    ...
                    self[key] = function () {
                        ...
                        try {
                            ...
                            if (utils.isUsable(aspects.afterThrowing[e.name], 'as_array')) {
                                utils.iterate(aspects.afterThrowing[e.name], function (aspect) {
                                    var execEventObject = execEvent.newExecutionEventObject();
                                    var paramMap = utils.getInstanceOf().object();
                                    paramMap.original = originalObject;
                                    paramMap.args = args;
                                ...
                                aspects.afterThrowing[e.name] = maintainAspects;
                                maintainAspects = utils.getInstanceOf().array();
                            } else {
                                throw e;
                            }
            ...
            this.aspectAfterThrowing = function (item, aspect) {
                if (utils.isNotUsable(aspect, 'as_function')) {
                    throw new TypeError('This method requires a valid function as the second parameter.');
                }
                ...
                if (item instanceof Error) {
                    errorName = item.name;
                } else if (utils.isUsable(item, 'as_string', 'has_content')) {
                    errorName = item;
                } else {
                    throw new TypeError('This method requires either an Error object or a valid String as the first parameter.');
                }

                if (utils.isNotUsable(aspects.afterThrowing[errorName], 'as_array')) {
                    aspects.afterThrowing[errorName] = utils.getInstanceOf().array();
                }

                aspects.afterThrowing[errorName].push(aspect);
